  ...
  if (items.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-6 py-24 text-center">
        <span className="material-symbols-outlined text-8xl text-slate-800 mb-6">shopping_cart</span>
        <h1 className="text-4xl font-black uppercase italic tracking-tighter mb-4">Your Cart is Empty</h1>
        <p className="text-slate-500 font-medium mb-10">Time to upgrade your digital experience.</p>
        <button 
          onClick={() => window.location.reload()} // Simplified navigation
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12 animate-in fade-in duration-700">
      <div className="flex items-baseline justify-between mb-12">
        <h1 className="text-5xl font-black tracking-tighter uppercase italic leading-none">Your <span className="text-primary not-italic">Cart</span></h1>
        <span className="text-slate-500 font-black uppercase tracking-widest text-xs">{items.length} items</span>
      </div>

      <div className="grid lg:grid-cols-12 gap-12">
        <div className="lg:col-span-8 space-y-6">
          {items.map(item => (
            <div key={item.id} className="bg-surface-dark border border-white/5 rounded-2xl p-6 flex gap-6 items-center">
              ...
              <div className="text-right min-w-[100px]">
                ...
                <p className="text-[10px] text-slate-600 font-bold uppercase tracking-widest">${item.price} EA</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="lg:col-span-4">
          <div className="bg-surface-dark border border-white/5 rounded-2xl p-8 sticky top-32">
            ...
            <div className="flex justify-between items-end mb-10">
              <div>
                <p className="text-[10px] text-slate-600 font-black uppercase tracking-widest mb-1">Total</p>
                <p className="text-4xl font-black text-primary tracking-tighter">${(subtotal * 1.08).toFixed(2)}</p>
              </div>
